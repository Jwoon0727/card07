        ...
        try {
            ...
            if (result.deletedCount === 1) {
                ...
                if (session && session.user.name === nameToDelete) {
                    ...
                    await fetch(`https://example.com/api/auth/signout`, { method: 'POST' }); // 클라이언트에서 로그아웃 처리
                    return res.status(200).json({ message: '유저와 관련된 정보가 삭제되었습니다. 로그아웃되었습니다.' });
                }
                return res.status(200).json({ message: '유저와 관련된 정보가 삭제되었습니다.' });
            } else {
                return res.status(404).json({ message: '유저를 찾을 수 없습니다.' });
            }
        } catch (error) {
